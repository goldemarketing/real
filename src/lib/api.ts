...
// 👇 لاحظ إننا كتبنا الدومين هنا عشان لو الـ env ضرب، ده يشتغل
...
// ✅ Updated to support FormData (if location has images in future)
  ...
  try {
    ...
    if (!(data instanceof FormData)) {
      ...
      Object.keys(data).forEach((key) => {
        ...
        if (key === 'image') {
          // 👇 Casting to any to satisfy TS (no overlap between number and Blob)
